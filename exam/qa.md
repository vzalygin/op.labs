...
Данный markdown-файлик содержит в себе разбор вопросов в вольном пересказе для экзамена по дисциплине "Основы программирования" кафедры ИУ6 потока 2022. Фактически это краткая выжимка теории со всего курса, с помощью которой можно _повторить_ содержание курса.

***Disclaimer: данный файл не претендует ни на какую-либо полноту и фактологическую точность. Для качетсвенной подготовки рекомендуется самостоятельно искать информацию в первоисточниках, а этот файл (и ему подобные) использовать, как альтернативный взгляд на курс.***
UPD: Добавлена лицензия MIT.
...
[Смотреть тут](https://github.com/pluttan/BMSTU-1/blob/master/%D0%9E%D0%9F/%D0%9F%D0%BE%D0%B4%D0%B3%D0%BE%D1%82%D0%BE%D0%B2%D0%BA%D0%B0%20%D0%BA%20%D1%8D%D0%BA%D0%B7%D0%B0%D0%BC%D0%B5%D0%BD%D1%83/examOP.pdf)
...
Использование указания `in …` в файлах с расширением pas не допустимо, т.к. модули, прописанные в файле проекта, считаются частью проекта, т.е. в других модулях можно просто указывать название модуля без `in`.
...
Файловые системы имеют древовидную структуру, где [листьями](https://ru.wikipedia.org/wiki/%D0%94%D0%B2%D0%BE%D0%B8%D1%87%D0%BD%D0%BE%D0%B5_%D0%B4%D0%B5%D1%80%D0%B5%D0%B2%D0%BE) считаются файлы/пустые папки, а промежуточными узлами - непустые папки.
...
Собственно, внутренне файл представляется как последовательность байт, где каждый байт кодирует 1 символ, строки разделены байтами, имеющими значения маркеров конца строки. В конце файла записан байт с значением маркера конца файла.
...
Определение процедурной декомпозиции я стащил из другого файлика с билетами.

> Процедурная декомпозиция - разбиение программмы на более маленькие подпрограмммы, решающие отдельные подзадачи.
...
- наследование: комната с объёмом наследуется от комнаты с площадью (кстати, это правильное применение наследования с точки зрения принципа подстановки Лисков)
- композиция: квартира. В ней может быть от 1 до 15 комнат (знаем заранее)
- композиция 2: объект `текст` может содержать от 1 до некоторого `nmax` объектов `слово`.
- наполнение: комната с балконом. Либо в ней есть балкон, либо нет.
- наполнение 2: объект `текст` может содержать неопределённое количество `слов` или не содержать.
...
К объектам можно создавать указатели, класть их в кучу, как и все остальные переменные. Для этого нужно использовать те же механизмы: типизированные указатели `^{class_name}`, процедуры `New` и `Dispose` для выделения и освобождения памяти в куче. Классы также могут иметь поля-указатели на динамические объекты.  
Для полиморфных методов при выделении памяти возможно вызвать конструктор с помощью синтаксиса `New({class_name}, {constructor_name})`, для корректного освобождения памяти из-под полимофного объекта нужно вызвать деструктор (`destructor`), причём доступен следующий синтаксис `Dispose({class_name}, {destructor_name})`. Конструкторы и деструкторы нужны для корректной работы с ТВМ. Если в объекте используются динамические поля, то выделение под них памяти и её освобождение осуществляется также в конструкторах и деструкторах.

### Также в этом билете возможно стоит рассказать о полиморфизме

> Сам по себе полиморфизм - особенность наследования, при которой методы потомка переопределяют методы родителя.

При _простом (статическом) полиморфизме_ тип объекта, у которого вызывается переопределённый метод, известен на этапе компиляции (он совпадает с типом переменной), а значит известен и сам метод (_раннее связывание_). Такие методы называются _статическими полиморфными_.

Для обращения к переопределённому методу родительского класса можно использовать ключевое слово `inherited`.  
При внешнем по отношению к классу вызове для обращения к методу родительского класса можно использовать явное приведение:

```pascal
type foo = object
            function e: integer; // возвращает 2
           end;
type bar = object(foo)
            function e: integer; // возвращает 3
           end;

var baz: bar;
begin
  WriteLn(baz.e); // Вывод: 3. Тип известен на этапе компиляции: bar
  WriteLn(foo(baz).e); // Вывод 2. Тип известен на этапе компиляции: foo
end.      
```

Такое приведение типа бывает восходящим (upcast, приводим к родительскому классу) и нисходящим (downcast, приводим к классу потомка). Для downcast нужно убедится, что переменная ссылатся на объект класса потомка.

Невсегда можно обойтись простым полиморфизмом. Существуют ситуации, когда тип переменной не может быть установлен во время компиляции (точнее может, но неверно с точки зрения логики программиста), в таких случаях нужно использовать _сложный полиморфизм_, при котором выполняется сопоставление адресов методов с их вызовами во время исполнения (_позднее связывание_). Методы, для которых выполняется позднее связывание, называются _виртуальными полиморфными_. При сложном полиморфизме для установления, какой метод нужно вызывать в данный момент времени работы программы, используется _таблица виртуальных методов_. Адрес таблицы для каждого класса неявно хранится вместе с данными объекта класса и весит _4 байта_. Для создания такой таблицы нужно вызвать _конструктор_ у переменной, иначе при попытке обратиться к виртуальному методу будет неприятно (зависание, паника и другие неприятные спецэффекты).

В связи со всем этим для использования сложного полиморфизма нужно сделать 3 вещи:

1. Переопределяемые методы нужно пометить служебным словом `virtual` (`function a.b: integer; virtual;`, если метод `b` класса `a` виртуальный).
2. Создать процедуру-конструктор (просто для процедуры, выполняющей инициализацию, заменить `procedure` на `constuctor`).
3. Вызвать конструктор до обращения к виртаульным методам.

3 ситуации, когда нужно использовать сложный полиморфизм:

1. Наследуемый метод для объекта производного класса вызывает метод, который переопределён в производном классе (при ранним связывании наследуемый метод будет обращаться к методу базового класса, а нужно - к методу производного).
2. Объект производного класса через указатель базового класса обращается к методу, переопределённому производным классом.
3. Процедура вызывает переопределённый метод для объекта производного класса, переданного в процедуру через параметр-переменную, описанный как объект базового класса (`процедура с полиморфным объектом`).

Примеры, демонстрирующие эти ситуации, можно посмотреть в учебнике по основа программирования Ивановой. В целом все три случая сводятся к одному: обращение к полиморфному методу выполняется через указатель базового класса, которому может быть присвоен адрес объекта не только базового, но и производного класса. Объект, адресуемый подобным указателем называется `полиморфным`.

Функция `TypeOf({имя класса или объекта}): pointer` возвращает указатель на ТВМ класса. В связи с этим возможно определение, принадлежит ли объект какому-либо классу:

```pascal
if TypeOf(Self) = TypeOf({class name}) then
  // Принадлежит
else
  // Не принадлежит
```

Некоторые особенности сложного полиморфизма:

1. Требует построения ТВМ, а значит и больше памяти
2. Вызовы виртуальных методов происходят через ТВМ, что замедляет программу
3. Список параметров виртуальных полиморфных методов должен совпадать, для статических полиморфных - необязательно.
4. Статический полиморфный метод не может переопределять виртуальный полиморфный метод.
...
Поскольку я не вижу смысла глубоко разбирать этот вопрос, из-за его не совсем понятного статуса (а что это вообще делает в курсе по основам программирования?), я стащу разбор этого билета у более [ответственных людей](https://github.com/pluttan/BMSTU-1/blob/master/%D0%9E%D0%9F/%D0%9F%D0%BE%D0%B4%D0%B3%D0%BE%D1%82%D0%BE%D0%B2%D0%BA%D0%B0%20%D0%BA%20%D1%8D%D0%BA%D0%B7%D0%B0%D0%BC%D0%B5%D0%BD%D1%83/examOP.pdf).

> **Событийно-ориентированное программирование (СОП)** – парадигма программирования, в которой выполнение
программы определяется событиями – действиями пользователя (мышь, клавиатура, сенсорный экран и тд), сообщениями
других программ и потоков, событиями операционной системы.  
**События операционной системы** – любые изменения состояния ОС: запуск системы (ПК), вход в систему любого
пользователя, любые запускаемые процессы и тд.

Порядок обработки событий ОС:

1. В системе происходит событие, например, пользователь передвинул мышь или нажал на клавишу клавиатуры, в резуль-
тате генерируется сообщение об этом событии – сообщение ОС.
...
5. Если в приложении предусмотрен соответствующий обработчик события среды, то он вызывается, если нет – то про-
должается обработка сообщения ОС.
