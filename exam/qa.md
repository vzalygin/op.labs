...
***Disclaimer: данный файл не претендует ни на какую-либо полноту и фактологическую точность. Для качетсвенной подготовки рекомендуется самостоятельно искать информацию из первоисточников, а этот файл и ему подобные использовать, как альтернативный взгляд на курс.***
...
Для полиморфных методов при выделении памяти возможно вызвать конструктор с помощью синтаксиса `New({указатель}, {вызов контруктора})`, для корректного освобождения памяти из-под полимофного объекта нужно вызвать деструктор (`destructor`), причём доступен следующий синтаксис `Dispose({указатель}, {вызов деструктора})`. Конструкторы и деструкторы нужны для корректной работы с ТВМ. Если в объекте используются динамические поля, то выделение под них памяти и её освобождение осуществляется также в конструкторах и деструкторах.
...
1. Переопределяемые методы нужно пометить служебным словом `virtual` (`function a.b: integer; virtual;`, если метод `b` и класса `a` виртуальный).
