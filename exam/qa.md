...
Данный markdown-файлик содержит в себе разбор вопросов для экзамена по дисциплине "Основы программирования" кафедры ИУ6 потока 2022. Фактически это краткая выжимка теории со всего курса. Я расписал те факты, которые считаю важными (моё мнение может отличаться от общепринятого) + дополнил своими замечаниями и комментариями, описал пару важных моментов, которые были пропущены во время чтения курса.
...
К объектам можно создавать указатели, класть их в кучу, как и все остальные переменные. Для этого нужно использовать те же механизмы: типизированные указатели `^{class_name}`, процедуры `New` и `Dispose` для выделения и освобождения памяти в куче.  
Классы также могут иметь поля-указатели на динамические объекты.  
В общем, механизмы абсолютно идентичны.
...
При _простом (статическом) полиморфизме_ тип объекта, у которого вызывается переопределённый метод, известен на этапе компиляции (он совпадает с типом переменной), а значит известен и сам метод (_раннее связывание_). Такие методы называются _статически полиморфными_.
...
var baz: bar;
begin
  baz.init // обращаемся к методу класса bar
  foo(baz).init // обращаемся к методу класса foo
end;
```
