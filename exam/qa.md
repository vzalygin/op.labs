...
Данный markdown-файлик содержит в себе разбор вопросов для экзамена по дисциплине "Основы программирования" кафедры ИУ6 потока 2022. Фактически это краткая выжимка теории со всего курса, с помощью которой можно _повторить_ содержание курса. Я расписал те факты, которые считаю важными + дополнил своими замечаниями и комментариями, описал пару вещей, которые были опущены во время чтения курса.
...
Собственно, внутренне файл представляется как последовательность байт, где каждый байт кодирует 1 символ (или каждые 2, в зависимости от кодировки), строки разделены байтами, имеющими значения маркеров конца строки. В конце файла записан байт с значением маркера конца файла.
